...
function Profil(props) {
  ...
  const [mode, setMode] = useState("message"); // Sélection du mode à afficher
  const [visible, setVisible] = useState(false); // gestion overlay
  const [unreadMessage, setUnreadMessage] = useState({ list: [] });
  const [notation, setNotation] = useState(false); // Overlay pour la notation
  ...
  useEffect(() => {
    ...
    request2
      ...
      .catch((err) => console.log("Requete appointement error: ", err));
  }, []);
  ...
  /**
   * Réduit le nombre de caractère d'un String
   * @param {String} word La chaine de caractère qu'il faut tronquer
   * @param {Number} nbr Le nombre de caractère maximum à avoir
   * @returns {String} Nouvelle chaine de caracteres tronquée
   */
